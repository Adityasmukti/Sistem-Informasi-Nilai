...
namespace Project_SINS.Laporan
{
    public partial class FInputNilai : Form
    {
        ...
        {
            if (e.ColumnIndex == DB.GetColumnIndexByName(dgnilai, "INPUT"))
            {
                if (dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "INPUT")].Value.ToString() == "HAPUS")
                {
                    if (MessageBox.Show("Yakin hapus nilai?", "Pertanyaan", MessageBoxButtons.YesNo, MessageBoxIcon.Question)==DialogResult.Yes)
                    {
                        query = "DELETE FROM tbl_nilai WHERE id_nilai=" + dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "ID NILAI")].Value.ToString(); ;
                        DM.ManipulasiData(query);
                    }
                }
                else
                {
                    query = "SELECT id_ruangan FROM tbl_ruangan WHERE id_siswa=" + dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "ID SISWA")].Value.ToString()+
                    " AND id_kelas=" + listidkelas[cbkelas.SelectedIndex]+" LIMIT 1";
                    idruangan = DM.singeldata(query) ;
                    string values = dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "NILAI")].Value.ToString();
            ...
            else if (e.ColumnIndex == DB.GetColumnIndexByName(dgnilai, "KETERANGAN"))
            {
                string values = dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "KETERANGAN")].Value.ToString();
                if (dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "INPUT")].Value.ToString() == "HAPUS")
                {
                    if (StringExtensions.InputTextBox("Input", "Keterangan", ref values) == DialogResult.OK)
                    {
                        query = "UPDATE tbl_nilai SET ketsiswa='" + values + "' WHERE id_nilai=" + dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "ID NILAI")].Value.ToString();
                        DM.ManipulasiData(query);
                        dgnilai.CurrentRow.Cells[DB.GetColumnIndexByName(dgnilai, "KETERANGAN")].Value = values;
                    }
                }
                else
