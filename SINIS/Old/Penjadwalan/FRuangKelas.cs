            ...
            if (e.ColumnIndex == 0)
            {
                ...
                if (dgsiswa.CurrentRow.Cells[0].Value.ToString() == "SUDAH")
                {
                    query = "DELETE FROM tbl_ruangan WHERE id_ruangan=" +
                        dgsiswa.CurrentRow.Cells[ExtensionMethods.DB.GetColumnIndexByName(dgsiswa, "ID RUANGAN")].Value.ToString();
                ...
                else
                {
                    ...
                    query = "SELECT * FROM tbl_ruangan WHERE tahunajaran='"+cbtahunajaran.Text+"' AND id_siswa=" +
                        dgsiswa.CurrentRow.Cells[ExtensionMethods.DB.GetColumnIndexByName(dgsiswa, "ID SISWA")].Value.ToString();
                    foreach (DataRow br in DM.GetData(query).Tables[0].Rows)
                    {
                    ...
                    {
                        string nis = dgsiswa.CurrentRow.Cells[ExtensionMethods.DB.GetColumnIndexByName(dgsiswa, "NIS")].Value.ToString();
