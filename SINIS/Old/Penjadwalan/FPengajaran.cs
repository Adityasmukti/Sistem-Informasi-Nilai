            ...
            if (e.ColumnIndex == 0)
            {
                ...
                if (dgpengajaran.CurrentRow.Cells[0].Value.ToString() == "SUDAH")
                {
                    query = "DELETE FROM tbl_jadwal WHERE id_jadwal=" +
                        dgpengajaran.CurrentRow.Cells[ExtensionMethods.DB.GetColumnIndexByName(dgpengajaran, "ID JADWAL")].Value.ToString();
                ...
                else
                {
                    ...
                    query = "SELECT * FROM tbl_jadwal WHERE tahunajaran='" + cbtahunajaran.Text + "' AND id_kelas=" +
                        dgpengajaran.CurrentRow.Cells[ExtensionMethods.DB.GetColumnIndexByName(dgpengajaran, "ID KELAS")].Value.ToString()+
                        " AND id_pelajaran="+ dgpengajaran.CurrentRow.Cells[ExtensionMethods.DB.GetColumnIndexByName(dgpengajaran, "ID PELAJARAN")].Value.ToString();
                    foreach (DataRow br in DM.GetData(query).Tables[0].Rows)
                    {
                    ...
                    if (belumdapat)
                    {
                        query = "INSERT INTO tbl_jadwal (id_pelajaran, id_kelas,id_guru, tahunajaran) VALUES (" +
                            dgpengajaran.CurrentRow.Cells[ExtensionMethods.DB.GetColumnIndexByName(dgpengajaran, "ID PELAJARAN")].Value.ToString() + ", " +
